...
const getStatusBadge = (status) => {
    switch (status) {
        case 'processing':
            return { label: 'جاري التحميل', class: 'bg-indigo-50 text-indigo-600 border-indigo-200' };
        case 'failed':
            return { label: 'فشل', class: 'bg-rose-50 text-rose-600 border-rose-200' };
        case 'completed':
            return { label: 'مكتمل', class: 'bg-emerald-50 text-emerald-600 border-emerald-200' };
        default:
            return { label: 'معلق (Idle)', class: 'bg-amber-50 text-amber-600 border-amber-200' };
    }
};
...
const handleDelete = (id) => {
    if (!id) {
        notiStore.triggerNotification('لا يوجد تاسك لحذفة')
        return
    }

    confirmAndDelete({
        message: 'هل أنت تأكد من حذف هذا التاسك؟',
...
<template>
    <div class="bg-card border border-line rounded-2xl shadow-soft overflow-hidden">
        ...
        <div v-else-if="taskStore.allTasks && taskStore.allTasks.length > 0" class="w-full overflow-x-auto">
            <table class="w-full text-right text-fluid-xs whitespace-nowrap">
                ...
                <tbody class="divide-y divide-line/60 bg-card text-main">
                    <tr v-for="task in taskStore.allTasks" :key="task.id"
                        ...
                        <!-- اسم التاسك والمصدر -->
                        <td class="px-4 py-3">
                            <div class="font-bold text-main leading-snug max-w-xs sm:max-w-md truncate" :title="task.task_name">
                                {{ task.task_name }}
                            </div>
                            <div class="text-[11px] text-sub flex items-center gap-2 mt-1">
                                <span>السنة: <strong class="text-main font-mono">{{ task.task_year || '-' }}</strong></span>
                                <span>•</span>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <button type="button" @click="handleEdit(task.id)"
                                ...
                                <button type="button" @click="handleDelete(task.id)"
        ...
        <div v-else
            class="text-fluid-xs text-sub py-12 text-center bg-line/5 space-y-3">
            ...
            <p class="font-semibold text-main">لا توجد مهام لعرضها حالياً</p>
            <p class="text-sub">قم بإضافة مهام جديدة لتبدأ بالظهور هنا.</p>
        </div>
