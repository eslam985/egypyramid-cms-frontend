...
<template>
  <div class="bg-card border border-line rounded-2xl shadow-soft overflow-hidden">
    ...
    <div v-else-if="taskStore.allTasks && taskStore.allTasks.length > 0" class="w-full overflow-x-auto">
      <table class="w-full text-start text-fluid-xs whitespace-nowrap">
        <thead class="bg-line/10 border-b border-line text-sub font-bold uppercase">
          <tr>
            <th class="p-fluid whitespace-nowrap text-center">{{ t('tasks.table.id') }}</th>
            <th class="p-fluid whitespace-nowrap text-center">{{ t('tasks.table.taskName') }}</th>
            <th class="p-fluid whitespace-nowrap text-center">{{ t('tasks.table.status') }}</th>
            <th class="p-fluid whitespace-nowrap text-center">{{ t('tasks.table.progress') }}</th>
            <th class="p-fluid whitespace-nowrap text-center">{{ t('tasks.table.systemMessage') }}</th>
            <th class="p-fluid whitespace-nowrap text-center">{{ t('tasks.table.createdAt') }}</th>
            <th class="p-fluid whitespace-nowrap text-center">{{ t('tasks.table.actions') }}</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-line/60 bg-card text-main">
          <tr v-for="task in taskStore.allTasks" :key="task.id" class="hover:bg-line/10 transition-colors duration-150">
            <!-- ID -->
            <td class="p-fluid font-mono text-sub text-center">
              {{ task.id }}
            </td>

            <!-- اسم التاسك والمصدر -->
            <td class="p-fluid whitespace-nowrap text-center">
              <div class="font-bold flex justify-center leading-snug max-w-xs sm:max-w-md truncate text-center"
                :title="task.task_name">
                {{ task.task_name }}
              </div>
              <div class="text-text-fluid-xs text-sub flex items-center gap-2 mt-1">
                <span>{{ t('tasks.table.year') }}:
                  <strong class="text-main font-mono">{{ task.task_year || '-' }}</strong></span>
                <span>•</span>
                <a v-if="task.source_url" :href="task.source_url" target="_blank" rel="noopener noreferrer"
            ...
            <td class="p-fluid whitespace-nowrap text-center">
              <span :class="[
                'px-2.5 py-1 rounded-md text-text-fluid-xs font-semibold border inline-flex items-center gap-1.5',
            ...
            <!-- شريط التقدم والسرعة -->
            <td class="p-fluid whitespace-nowrap text-center">
              <div class="flex items-center justify-between text-text-fluid-xs font-bold mb-1">
            ...
            <!-- رسالة النظام -->
            <td class="p-fluid max-w-xs truncate text-center" :title="task.status_message">
            ...
            <td class="p-fluid text-center">
              <div class="flex items-center justify-center gap-fluid-gap">
                <button type="button" @click="handleEdit(task.id)" class="btn-outline" title="تعديل">
                ...
                <button type="button" @click="handleDelete(task.id)" class="btn-danger" title="حذف">
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div v-else class="text-fluid-xs text-sub py-12 text-center bg-line/5 space-y-3">
      <div class="w-12 h-12 mx-auto rounded-xl bg-line/20 flex items-center justify-center text-sub border border-line">
