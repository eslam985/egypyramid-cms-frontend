...
const BrokenLinksTable = defineAsyncComponent(() => import('@/components/dashbord/tabs/BrokenLinksTable.vue'))
const MissingEpisodesTable = defineAsyncComponent(() => import('@/components/dashbord/tabs/MissingEpisodesTable.vue'))
const NotReadyMediasTable = defineAsyncComponent(() => import('@/components/dashbord/tabs/NotReadyMediasTable.vue'))
const TelegramLockedTable = defineAsyncComponent(() => import('@/components/dashbord/tabs/TelegramLockedTable.vue'))

import { useAnalyticsStore } from '@/stores/analyticsStore'

const analyticsStore = useAnalyticsStore()
const { mediasNotReadyList, brokenLinksList, telegramLocked, missingLinksByServer } = storeToRefs(analyticsStore)
    ...
    <div class="rounded-2xl mb-6 md:p-fluid space-y-4 shadow-soft">
        ...
        <div class="grid grid-cols-2 md:flex items-center gap-4 py-fluid">
            ...
            <button @click="switchTab('notReady')"
                ...
                <span>الأعمال غير الجاهزة ({{ mediasNotReadyList.length }})</span>
            ...
            <button @click="switchTab('broken')"
                ...
                :class="activeTab === 'broken'
                    ? 'bg-danger! text-white! shadow-soft hover:brightness-110'
                    : 'bg-line/20 border border-line text-sub hover:text-main hover:bg-line/40'">
                <span>الروابط المكسورة ({{ brokenLinksList.length }})</span>
            ...
            <button @click="switchTab('telegram')"
                ...
                :class="activeTab === 'telegram'
                    ? 'bg-danger! text-white! shadow-soft hover:brightness-110'
                    : 'bg-line/20 border border-line text-sub hover:text-main hover:bg-line/40'">
                <span>روابط تليجرام المقفولة ({{ telegramLocked.length }})</span>
            ...
            <button @click="switchTab('missing')"
                ...
                :class="activeTab === 'missing'
                    ? 'bg-danger! text-white! shadow-soft hover:brightness-110'
                    : 'bg-line/20 border border-line text-sub hover:text-main hover:bg-line/40'">
                <span>الحلقات المفقودة ({{ missingLinksByServer.length }})</span>
