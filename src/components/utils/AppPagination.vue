...
<template>
    <div v-if="totalPages > 0"
        ...
        <div class="text-sub font-medium">
            ...
            (إجمالي <span class="font-semibold text-accent">{{ totalItems }}</span> عنصر)
        </div>

        <!-- أزرار التنقل -->
        <div class="flex items-center gap-3 dir-ltr ">
            <!-- زر التالي -->
            <button type="button" @click="goToPage(currentPage + 1)" :disabled="currentPage === totalPages || isLoading"
            ...
            <template v-for="(p, index) in visiblePages">
                ...
                <button v-else :key="p" type="button" @click="goToPage(p)" :disabled="isLoading" :class="[
            ...
            <!-- زر السابق -->
            <button type="button" @click="goToPage(currentPage - 1)" :disabled="currentPage === 1 || isLoading"
                class="p-fluid rounded-lg border border-custom-border font-semibold text-sub hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer">
                السابق
            </button>
        </div>
