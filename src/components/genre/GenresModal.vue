...
const handleSubmit = async () => {
    ...
    if (isSuccess) {
        const msg = genreId ? 'تم تعديل التصنيف بنجاح' : 'تم إضافة التصنيف بنجاح'
        notiStore.triggerNotification(genresStore.successMessage || msg)
        router.push({ name: 'genresList' })
    } else {
        notiStore.triggerNotification(genresStore.errorMessage || 'حدث خطأ أثناء الحفظ')
    }
}
    ...
    <div class="max-w-xl mx-auto card my-6 shadow-soft">
        ...
        <div class="mb-6 pb-4 border-b border-line">
            <h2 class="text-fluid-h3 text-title">
                {{ genreId ? 'Edit Genre' : 'Add New Genre' }}
            </h2>
            <p class="text-fluid-xs text-sub mt-1">
                {{ genreId ? 'تعديل بيانات التصنيف الحالي' : 'إضافة تصنيف جديد لنظام الميديا' }}
            </p>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" id="name" v-model="formData.name" placeholder="مثال: أكشن، دراما، كوميدي"
            ...
            <div class="pt-4 border-t border-line flex items-center justify-end gap-3">
                ...
                <button type="submit" class="btn-primary" :disabled="genresStore.isLoading">
                    <span v-if="genresStore.isLoading" class="flex items-center gap-2">
                        <svg class="animate-spin h-4 w-4 text-on-accent" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24">
                    ...
                    <span v-else>{{ genreId ? 'Update Genre' : 'Add Genre' }}</span>
                </button>
            </div>
        </form>
    </div>
