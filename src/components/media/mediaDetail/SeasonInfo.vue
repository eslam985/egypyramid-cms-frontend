...
const handleDeleteSeason = async (seasonId) => {
    ...
    confirmAndDelete({
        message: `are you sure to delete season id ${seasonId}`,
    ...
    <main class="space-y-6">
        ...
        <div v-else class="bg-card rounded-2xl border border-line p-4 md:p-6 shadow-soft space-y-6">
            ...
            <div
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-fluid-h2 font-bold text-main">قائمة المواسم</h2>
                        <p class="text-fluid-xs text-sub">إدارة المواسم والتنقل بين الحلقات الخاصة بها</p>
                    </div>
                </div>
            ...
            <div v-if="mediaContentStore.seasons && mediaContentStore.seasons.length > 0 && isSeries"
                class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">

                <div v-for="season in mediaContentStore.seasons" :key="season.id" :class="[
                    ...
                    mediaContentStore.currentSeason?.id === season.id ? 'border-accent bg-accent/10 ring-2 ring-accent/30' : 'border-line hover:border-accent/50'
                ]">

                    <!-- الجزء العلوي: رقم الموسم وعدد الحلقات -->
                    <div @click="handleSelectSeason(season.id)"
                        ...
                        <div class="px-2.5 py-1 rounded-full text-fluid-xs font-semibold bg-line/20 border border-line flex items-center gap-1 shrink-0"
                            title="عدد الحلقات المضافة لهذا الموسم">
                            <template v-if="season.episodes_count > 0">
                                <span class="text-sub">حلقة</span>
                                <span class="text-success font-bold">{{ season.episodes_count }}</span>
                            </template>
                            <template v-else>
                                <span class="text-danger text-[11px]">بدون حلقات</span>
                            </template>
                        </div>
                    </div>

                    <!-- تفاصيل إضافية -->
                    <div class="pt-2 border-t border-line/60 text-fluid-xs flex items-center justify-between text-sub">
                        <span>المعرّف:</span>
                        <code
        ...
        {{ season.id }}
                    ...
                    <div class="pt-2 flex items-center justify-end gap-1 border-t border-line/40">
                        <button type="button" @click="handleEditSeason(season.id)"
                        ...
                        <button type="button" @click="handleDeleteSeason(season.id)"
            ...
            <div v-else
                ...
                <p class="font-medium">{{ isSeries ? 'لا توجد مواسم مضافة لهذا العمل بعد.' : '' }}</p>
            </div>
