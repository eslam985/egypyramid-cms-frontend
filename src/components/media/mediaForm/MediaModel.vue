...
const handleSubmit = async () => {
  ...
  if (result) {
    const msg = mediaId ? t('media.form.updatedSuccess') : t('media.form.addedSuccess')
    notiStore.triggerNotification(mediaStore.successMessage || msg)
    router.push({ name: 'mediaList' })
  } else {
    notiStore.triggerNotification(mediaStore.errorMessage || t('media.form.saveError'))
  }
}
...
onMounted(async () => {
  ...
  if (mediaId) {
    ...
    if (!foundMedia && !mediaStore.currentMedia) {
      notiStore.triggerNotification(genresStore.errorMessage || t('media.form.notFound'))
      router.push({ name: 'NotFound' })
    }
    ...
    if (mediaStore.currentMedia) {
      ...
      if (Array.isArray(mediaStore.currentMedia.genres)) {
        formData.genres = mediaStore.currentMedia.genres.map((g) =>
  ...
  <main class="max-w-5xl mx-auto my-3 md:my-8 card p-6 md:p-8 shadow-soft">
    ...
    <div class="mb-6 pb-4 border-b border-line flex flex-col md:flex-row items-center justify-between gap-4">
      <div>
        ...
        <p v-if="mediaId" class="text-fluid-xs text-sub mt-1">
          {{ t('media.form.editSubtitle') }}
        </p>
        <p v-else class="text-fluid-xs text-sub mt-1">
          {{ t('media.form.addSubtitle') }}
        </p>
      </div>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-fluid-gap">
        ...
        <div class="space-y-4">
          <div>
            <label for="name" class="form-label">{{ t('media.form.titleLabel') }} <span
                class="text-danger">*</span></label>
            <input type="text" id="name" name="name" class="form-input" :placeholder="t('media.form.titlePlaceholder')"
              required autocomplete="name" v-model="formData.title" />
            <p v-if="errors.title" class="form-error">{{ errors.title }}</p>
          </div>

          <div>
            <label for="year" class="form-label">{{ t('media.form.yearLabel') }} <span
                class="text-danger">*</span></label>
            <input type="text" id="year" name="year" class="form-input" placeholder="2026" autocomplete="year" required
              v-model="formData.year" />
            <p v-if="errors.year" class="form-error">{{ errors.year }}</p>
          </div>

          <div>
            <label for="tmdb_id" class="form-label">{{ t('media.form.tmdbIdLabel') }}</label>
            <input type="text" id="tmdb_id" name="tmdb_id" class="form-input" placeholder="196322 أو tt39741670"
              autocomplete="tmdb_id" v-model="formData.tmdb_id" />
            <p v-if="errors.tmdb_id" class="form-error">{{ errors.tmdb_id }}</p>
          </div>

          <div>
            <label for="poster_url" class="form-label">{{ t('media.form.posterUrlLabel') }}</label>
            <input type="text" id="poster_url" name="poster_url" class="form-input"
              placeholder="https://res.cloudinary.com/..." autocomplete="poster_url" v-model="formData.poster_url" />
            <p v-if="errors.poster_url" class="form-error">{{ errors.poster_url }}</p>
          </div>

          <div>
            <label for="story" class="form-label">{{ t('media.form.storyLabel') }}</label>
            <textarea dir="rtl" id="story" name="story" rows="4" class="form-textarea resize-none"
              :placeholder="t('media.form.storyPlaceholder')" autocomplete="story" v-model="formData.story"></textarea>
            <p v-if="errors.story" class="form-error">{{ errors.story }}</p>
          </div>

          <div>
            <label for="rating" class="form-label">{{ t('media.form.ratingLabel') }}</label>
          ...
          <div>
            <span class="form-label">{{ t('media.form.genresLabel') }}</span>
            <div
              ...
              <label v-for="genre in genresStore.allGenres" :key="genre.id"
                ...
                <input type="checkbox" :value="genre.id" class="accent-accent size-4 rounded cursor-pointer"
        ...
        <div class="space-y-4">
          <div>
            <label for="runtime" class="form-label">{{ t('media.form.runtimeLabel') }}</label>
            <input type="text" id="runtime" name="runtime" class="form-input"
              :placeholder="t('media.form.runtimePlaceholder')" autocomplete="runtime" v-model="formData.runtime" />
            <p v-if="errors.runtime" class="form-error">{{ errors.runtime }}</p>
          </div>

          <div>
            <label for="duration_iso" class="form-label">{{ t('media.form.durationIsoLabel') }}</label>
            <input type="text" id="duration_iso" name="duration_iso" class="form-input" placeholder="PT01H57M"
              autocomplete="duration_iso" v-model="formData.duration_iso" />
            <p v-if="errors.duration_iso" class="form-error">{{ errors.duration_iso }}</p>
          </div>

          <div>
            <label for="labels" class="form-label">{{ t('media.form.labelsLabel') }}</label>
            <input type="text" id="labels" name="labels" class="form-input text-right"
              :placeholder="t('media.form.labelsPlaceholder')" autocomplete="labels" v-model="formData.labels" />
            <p v-if="errors.labels" class="form-error">{{ errors.labels }}</p>
          </div>

          <!-- Category Radio Group -->
          <div>
            <span class="form-label">{{ t('media.form.categoryLabel') }} <span class="text-danger">*</span></span>
            <div class="flex items-center gap-6 mt-2 p-3 bg-card-hover rounded-xl border border-line">
              <label for="Category_tv"
                class="flex items-center gap-2 cursor-pointer text-fluid-xs font-medium text-title">
                <input type="radio" id="Category_tv" name="Category" value="tv" class="accent-accent size-4" required
                  v-model="formData.category" />
                {{ t('media.form.categoryTv') }}
              </label>
              <label for="Category_movie"
                class="flex items-center gap-2 cursor-pointer text-fluid-xs font-medium text-title">
                <input type="radio" id="Category_movie" name="Category" value="movie" class="accent-accent size-4"
                  v-model="formData.category" />
                {{ t('media.form.categoryMovie') }}
              </label>
            </div>
          ...
          <div>
            <span class="form-label">{{ t('media.form.mediaTypeLabel') }} <span class="text-danger">*</span></span>
            <div class="flex items-center gap-6 mt-2 p-3 bg-card-hover rounded-xl border border-line">
              <label for="type_series"
                class="flex items-center gap-2 cursor-pointer text-fluid-xs font-medium text-title">
                <input type="radio" id="type_series" name="media_type" value="series" class="accent-accent size-4"
                  required v-model="formData.media_type" />
                {{ t('media.form.typeSeries') }}
              </label>
              <label for="type_movie"
                class="flex items-center gap-2 cursor-pointer text-fluid-xs font-medium text-title">
                <input type="radio" id="type_movie" name="media_type" value="movie" class="accent-accent size-4"
                  v-model="formData.media_type" />
                {{ t('media.form.typeMovie') }}
              </label>
            </div>
          ...
          <div>
            <label for="is_ready"
              ...
              <span class="text-fluid-xs font-semibold text-title">{{ t('media.form.isReadyLabel') }}</span>
              <input type="checkbox" id="is_ready" name="is_ready" class="accent-accent size-5 rounded cursor-pointer"
                v-model="formData.is_ready" />
            </label>
          ...
          <div>
            <label for="is_facebook_posted"
              class="flex items-center justify-between p-3 bg-card-hover rounded-xl border border-line cursor-pointer hover:bg-card transition-colors">
              <span class="text-fluid-xs font-semibold text-title">{{ t('media.form.isFacebookPostedLabel') }}</span>
              <input type="checkbox" id="is_facebook_posted" name="is_facebook_posted"
                class="accent-accent size-5 rounded cursor-pointer" v-model="formData.is_facebook_posted" />
            </label>
            <p v-if="errors.is_facebook_posted" class="form-error">
              {{ errors.is_facebook_posted }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="pt-6 border-t border-line flex items-center justify-end gap-3">
        ...
        <button type="submit" class="btn-primary" :disabled="mediaStore.isLoading">
          ...
          <span v-else>{{ mediaId ? t('media.form.updateBtn') : t('media.form.addBtn') }}</span>
        </button>
      </div>
    </form>
  </main>
