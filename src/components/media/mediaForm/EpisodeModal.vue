...
const currentSchema = computed(() =>
  route.name === 'addEpisode' ? createEpisodeSchema : updateEpisodeSchema,
)
...
onMounted(async () => {
  if (route.name === 'addEpisode') {
    ...
    if (!media && !mediaStore.currentMedia) {
      notiStore.triggerNotification(t('media.episodeForm.mediaNotFound'))
...
const handleSubmit = async () => {
  ...
  const isSuccess =
    route.name === 'addEpisode'
      ? await mediaContentStore.addEpisode(mediaId, data)
      : await mediaContentStore.editEpisodeById(episodeId, data)

  if (isSuccess) {
    const msg = route.name === 'addEpisode' ? t('media.episodeForm.addedSuccess') : t('media.episodeForm.updatedSuccess')
    notiStore.triggerNotification(mediaContentStore.successMessage || msg)
    router.push({ name: 'mediaDetails' })
  } else {
    notiStore.triggerNotification(mediaContentStore.errorMessage || t('media.episodeForm.saveError'))
  }
}
</script>
<template>
    ...
    <div class="max-w-xl mx-auto card my-6 shadow-soft">
      ...
      <div class="mb-6 pb-4 border-b border-line">
        <h2 class="text-fluid-h3 text-title">
          {{ route.name === 'addEpisode' ? t('media.episodeForm.addTitle') : t('media.episodeForm.editTitle') }}
        </h2>
        <p class="text-fluid-xs text-sub mt-1">
          {{
            route.name === 'addEpisode'
              ? t('media.episodeForm.addSubtitle')
              : t('media.episodeForm.editSubtitle')
          }}
        </p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-5">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-fluid-gap">
          ...
          <div>
            <label for="episode_number" class="form-label">
              {{ t('media.episodeForm.episodeNumberLabel') }} <span class="text-danger">*</span>
            </label>
            <input id="episode_number" type="number" v-model.number="formData.episode_number" min="1"
              :placeholder="t('media.episodeForm.episodeNumberPlaceholder')" class="form-input" required />
          ...
          <!-- اختيار الموسم -->
          <div v-if="isSeries">
            <label for="season_id" class="form-label">{{ t('media.episodeForm.seasonLabel') }}</label>
            <select id="season_id" v-model="formData.season_id" class="form-select">
              <option v-if="!mediaContentStore.seasons.length" disabled :value="null">
                {{ t('media.episodeForm.noSeasons') }}
              </option>

              <option v-for="season in mediaContentStore.seasons" :key="season.id" :value="season.id">
                {{ t('media.seasonInfo.seasonLabel', { number: season.season_number }) }}
              </option>
            </select>
          ...
          <!-- نسبة التقدم -->
          <div>
            <label for="progress_percent" class="form-label"> {{ t('media.episodeForm.progressLabel') }} </label>
            <input id="progress_percent" type="number" v-model.number="formData.progress_percent" min="0" max="100"
              placeholder="0 - 100" class="form-input" />
          ...
          <!-- الحالة -->
          <div>
            <label for="status" class="form-label"> {{ t('media.episodeForm.statusLabel') }} </label>
            <input id="status" type="text" v-model="formData.status"
              :placeholder="t('media.episodeForm.statusPlaceholder')" class="form-input" />
          ...
          <!-- رسالة الحالة -->
          <div>
            <label for="status_message" class="form-label"> {{ t('media.episodeForm.statusMessageLabel') }} </label>
            <input id="status_message" type="text" v-model="formData.status_message"
              :placeholder="t('media.episodeForm.statusMessagePlaceholder')" class="form-input" />
          ...
          <!-- سرعة التحميل -->
          <div>
            <label for="download_speed" class="form-label"> {{ t('media.episodeForm.downloadSpeedLabel') }} </label>
            <input id="download_speed" type="text" v-model="formData.download_speed"
              :placeholder="t('media.episodeForm.downloadSpeedPlaceholder')" class="form-input" />
          ...
          <div class="sm:col-span-2">
            <label for="download_url" class="form-label"> {{ t('media.episodeForm.downloadUrlLabel') }} </label>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-line">
          ...
          <button type="submit" class="btn-primary" :disabled="mediaContentStore.isLoading">
            ...
            <span v-else>
              {{ route.name === 'addEpisode' ? t('media.episodeForm.saveBtn') : t('media.episodeForm.updateBtn') }}
            </span>
          </button>
        </div>
      </form>
