...
const emptyToNull = (schema) =>
    z.preprocess((val) => (typeof val === 'string' && val.trim() === '' ? null : val), schema.nullable().optional());
...
export const createMediaSchema = z.object({
    ...
    labels: emptyToNull(z.string().trim().regex(labelRe, "التصنيفات يجب أن تكون كلمات مفصولة بفواصل")),
    duration_iso: emptyToNull(z.string().trim().regex(durationIsoRegex, "صيغة duration_iso غير صحيحة (مثال: PT1H30M)")),
    title: z.string().trim().min(3, "العنوان يجب ألا يقل عن 3 أحرف").max(100),
    year: z.string().trim().length(4,  "السنة يجب أن تكون 4 أرقام"),
    is_ready: z.preprocess((val) => val === true || val === 'true' || val === 'on', z.boolean()).default(false),
    is_facebook_posted: z.preprocess((val) => val === true || val === 'true' || val === 'on', z.boolean()).default(false),
