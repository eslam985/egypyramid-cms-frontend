...
<template>
  <div class="min-h-screen bg-background flex items-center justify-center p-fluid-p">
    <div
      class="max-w-md w-full bg-card-bg border border-custom-border rounded-2xl p-8 text-center shadow-sm"
    >
      <div
      ...
      <h1 class="text-fluid-h2 font-bold text-title mb-3">{{ t('serverError.title') }}</h1>
      <p class="text-sub text-fluid-p mb-8">
        {{ t('serverError.message') }}
      </p>
      <button @click="handleRetry" class="btn-primary w-full">{{ t('serverError.retry') }}</button>
    </div>
  </div>
</template>
